...
- Prepare `1.1.1` with skill-driven real Minimal and Standard scans, a verified versioned consumer runtime bootstrap, persistent user-level tool/result caches, and explicit separation between assessment, local scanning, and CI adoption.
...
- Add managed `vibesec scan --install-tools` routing, outside-repository results, private temporary raw scanner output, complete exit-category preservation, offline tests, and v1.1.1 release/upgrade documentation.
- Disclose managed scanner storage, cache reuse, Docker/Checkov, network, and privacy implications before approval, and add deterministic stderr-only runtime/tool installation progress without contaminating JSON output.
- Accept legitimate bounded Cosign/Sigstore signature bundles whose certificate or transparency evidence exceeds the generic JSON string limit, while preserving the 2 MiB file cap, strict structural validation, and atomic publication.
