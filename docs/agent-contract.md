...
Scanner results preserve the established VibeSec categories: 0 is success, 1 is a policy violation, 2 is a scanner or runtime failure, and 3 is invalid configuration or malformed input. Coverage is one of `ran`, `not_applicable`, `not_configured`, or `tool_error`; a failed tool cannot become a clean result.
