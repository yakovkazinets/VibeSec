...
The trusted assets are the installed harness, fixed Schemathesis image digest, command builder, limits, policy, and artifact validator. The OpenAPI document, generated inputs, API responses, and target image behavior are untrusted. Unauthenticated operation permits only manual and scheduled repository-owned workflow configuration. The separate static-bearer mode is schedule-only and scopes its one opaque secret to the scanner step; all other secrets, arbitrary headers, proxy settings, hooks, custom formats, external configuration, custom commands, and public targets remain prohibited.
