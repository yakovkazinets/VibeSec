...
`.github/workflows/ci.yml` protects VibeSec Guardian itself. The consumer starters require the accompanying local support directories and never download mutable VibeSec Guardian code at runtime. The Standard starter separates code under test from code controlling the scan: it archives `scripts/`, `config/`, `policy/`, and `rules/` from the pull request base commit into a runner-temporary trusted harness, installs tools there, scans the checked-out tree, and writes reports to another runner-temporary directory. Push, schedule, and manual runs use their current trusted commit. VibeSec Guardian's own CI instead executes proposed implementation changes so they can be tested, but uses read-only permissions and no secrets.
