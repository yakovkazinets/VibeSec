...
VibeSec generates a restrictive canonical JSON document that is also valid YAML and validates it against one exact plan shape before mounting it. The only jobs, in order, are `spider`, `passiveScan-wait`, `report`, and `exitStatus`; the environment contains one same-origin context. The command is exactly `zap.sh -cmd -silent -autorun /zap/wrk/vibesec-zap-plan.yaml`. The target repository and workflow inputs cannot provide plans, fragments, jobs, scripts, options, proxies, authentication, headers, or URLs.
