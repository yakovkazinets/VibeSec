...
VibeSec Standard emits `finding-groups.json` and `prioritized-findings.json` in addition to `normalized.json`. The original scanner finding and version-1 fingerprint remain authoritative for baselines and suppressions. Correlation is a separate versioned view; it never deletes an original finding or silently migrates fingerprints.
...
- `scanner-exact`: identical scanner fingerprints;
