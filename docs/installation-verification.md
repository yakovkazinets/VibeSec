...
The API fuzzing add-on has `.vibesec/install-addon-api-fuzzing.json` plus a hashed `.vibesec/api-fuzzing.json`. It requires the API baseline add-on and `api_fuzzing_target=true`, verifies the exact bounded runtime, payload registry, independent policy and workflow set, and rejects values above the reviewed ceilings. Verification establishes installation integrity only; it does not establish that active testing ran or that an observed response proves an injection vulnerability.
