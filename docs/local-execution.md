...
Local execution never installs project dependencies. Paths are resolved before use, scanner diagnostics are bounded, and JSON output is available with `--json`. `--tool-dir` selects the already installed, verified scanner directory for `scan`; it does not install or discover tools from the target repository. A selected scanner mode that later fails is reported as failure; there is no fallback that could erase evidence.
