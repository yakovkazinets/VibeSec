...
`.vibesec/project-capabilities.json` is the authoritative declaration of which project scopes exist. Schema version 1 contains exactly 17 Boolean keys. Unknown or duplicate keys, non-Booleans, malformed or non-UTF-8 input, byte-order marks, oversized files, symlinks, and dependency conflicts fail closed.

`authenticated_security_testing=true` requires `authentication=true` and at least one of `dast_target=true` or `api_security_target=true`. Installation then asks only for the name of the GitHub Actions secret containing the static bearer token. The token value is never accepted. VibeSec Guardian itself declares both authentication capabilities false, so authenticated security testing is `not_applicable`.
...
Dependencies are strict: `dast_target=true` requires `web_application=true`; `api_security_target=true` requires both `api=true` and `container_image=true`; `api_fuzzing_target=true` requires `api=true`, `api_security_target=true`, and `container_image=true`; `public_runtime=true` and `authentication=true` each require either `web_application=true` or `api=true`. `web_application=true` with `dast_target=false` is valid. VibeSec Guardian never silently rewrites conflicting answers.
...
VibeSec Guardian itself declares `web_application=false`, `dast_target=false`, `api=false`, `api_security_target=false`, and `api_fuzzing_target=false`. Controlled HTTP and OpenAPI fixtures test optional machinery without turning this toolkit into a web/API product. VibeSec Guardian therefore reports all runtime add-ons as `not_applicable`. Enabling one later requires explicit valid capability answers and separate add-on initialization.
