...
Dependencies are strict: `dast_target=true` requires `web_application=true`; `api_security_target=true` requires both `api=true` and `container_image=true`; `api_fuzzing_target=true` requires `api=true`, `api_security_target=true`, and `container_image=true`; `public_runtime=true` and `authentication=true` each require either `web_application=true` or `api=true`. `web_application=true` with `dast_target=false` is valid. VibeSec never silently rewrites conflicting answers.
...
VibeSec itself declares `web_application=false`, `dast_target=false`, `api=false`, `api_security_target=false`, and `api_fuzzing_target=false`. Controlled HTTP and OpenAPI fixtures test optional machinery without turning this toolkit into a web/API product. VibeSec therefore reports all runtime add-ons as `not_applicable`. Enabling one later requires explicit valid capability answers and separate add-on initialization.
