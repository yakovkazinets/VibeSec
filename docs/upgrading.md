...
For API fuzzing upgrades, preserve `.vibesec/api-fuzzing.json`, `policy/api-fuzzing-baseline.json`, and `policy/api-fuzzing-suppressions.json`. Review the Schemathesis digest, payload registry, launcher, modes, method opt-ins, ceilings, schemas, normalizer, correlation rules, and workflow together. An upgrade must never enable fuzzing, injection testing, mutating methods, authentication, or a live workflow automatically.
