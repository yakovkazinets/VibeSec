# VibeSec Guardian v1 interface reference
...
| `vibesec.cli.doctor` | `stable` | installation doctor | Diagnose an installed VibeSec Guardian profile without changing it. | `--target`<br>`--profile minimal&#124;standard`<br>`--json` | — | — | 0 healthy; 1 actionable warnings; 2 verification failure; 3 invalid input; 4 infrastructure failure. |
...
| `vibesec.cli.scan` | `stable` | portable CLI | Run the Minimal or Standard profile against a local repository. | `--target`<br>`--results`<br>`--profile minimal&#124;standard`<br>`--execution-mode native&#124;container&#124;auto`<br>`--tool-dir`<br>`--install-tools`<br>`--cache-dir`<br>`--network-mode online&#124;offline`<br>`--json` | `normalized.json`<br>`coverage.json`<br>`policy-result.json`<br>`report.md` | `ran`<br>`not_applicable`<br>`not_configured`<br>`tool_error` | 0 clean; 1 policy violation; 2 scanner or runtime failure; 3 invalid input. |
