"""Canonical product-version parsing without Git or network access."""
...
def validate_version(value: object) -> str:
    ...
    try:
        ...
        raise VersionError("version must be valid Unicode") from exc
    if not value or len(encoded) > MAX_VERSION_BYTES - 1:
        raise VersionError("version must be nonempty and at most 64 UTF-8 bytes")
    if value != value.strip() or any(unicodedata.category(character) in {"Cc", "Cs"} for character in value):
        raise VersionError("version must not contain surrounding whitespace or controls")
    if not PRODUCT_VERSION.fullmatch(value):
        raise VersionError("version must use MAJOR.MINOR.PATCH or MAJOR.MINOR.PATCH-dev syntax")
    return value
