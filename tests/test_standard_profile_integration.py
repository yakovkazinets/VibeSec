...
''')
    ...
    def test_complete_orchestration_outputs_reports_coverage_and_sboms(self):
        ...
        self.assertEqual(groups["model"], "vibesec-finding-groups")
        self.assertEqual(priorities["model"], "vibesec-prioritized-findings")
        self.assertEqual({item["correlation_key"] for item in groups["groups"]},
                         {item["correlation_key"] for item in priorities["groups"]})
        self.assertIn("Prioritized finding groups", (self.results / "report.md").read_text(encoding="utf-8"))
